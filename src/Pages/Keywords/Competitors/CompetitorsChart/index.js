...
type Props = {
  selectedCompetitors: Set<string | null>,
  competitorsData: Map<string | null, Object>,
...
class CompetitorsChart extends Component<Props, State> {
  ...
  UNSAFE_componentWillReceiveProps(nextProps: Props) {
    ...
    const { competitorsData, isLoading, period, selectedCompetitors } = nextProps;
    ...
    const oldCompetitorsData = this.props.competitorsData;

    competitorsData.forEach((competitorData, competitorDomain) => {
      const seriesItem = chart.get(this.toCompetitorSeriesId(competitorDomain));
      if (!seriesItem && selectedCompetitors.has(competitorDomain)) {
        chart.addSeries(this.getChartSeriesItem(competitorData));
        redraw = true;
      } else if (seriesItem) {
        if (!selectedCompetitors.has(competitorDomain)) {
          seriesItem.remove();
          redraw = true;
        } else if (
          this.isSeriesItemUpdated(
            oldCompetitorsData.has(competitorDomain) &&
              oldCompetitorsData.get(competitorDomain).days,
            competitorData.days,
          )
        ) {
          seriesItem.remove();
  ...
  toCompetitorSeriesId = (domain: string | null) =>
    `competitor-series-item-${domain ? this.domainIdMap[domain] : ''}`;

  getChartSeries = () => {
    const { showFake, competitorsData } = this.props;
    ...
    return this.getNotesSeries().concat(
      (Array.from(competitorsData.values()) || []).map(this.getChartSeriesItem),
    );
  };
  ...
  getChartSeriesItem = (obj, idx = 0) => ({
    id: this.toCompetitorSeriesId(obj.domain),
    name: obj.domain,
    ...
    color: this.props.competitorsColors[obj.domain] || colors[idx % colors.length],
    visible: true,
  });
  ...
  updateIdMap = props => {
    const { competitorsData } = props;
    competitorsData.forEach((competitorData, competitorDomain) => {
      if (!competitorDomain) {
        return;
      }

      if (!this.domainIdMap[competitorDomain]) {
        this.domainIdMap[competitorDomain] = uniqueId();
      }
    });
  };
  ...
  render() {
    ...
    const config = {
      ...
      lang: {
        noData: t('No competitor rank data for the selected period'),
      },
      title: {
        text: '',
        style: {
...
export default injectIntl(CompetitorsChart);

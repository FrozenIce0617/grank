...
class AffiliateCommissionsInfiniteTable extends Component<Props, State> {
  state = {
    resetIndicator: 0,
  };
  ...
  queryDataFormatter = ({ affiliate: { payments } }) => {
    const filteredPayments = this.props.prepareData(
      payments.filter(p => p.paid).map(item => ({ ...item, type: this.getPaymentType(item) })),
    );
    return {
...
)(withApollo(AffiliateCommissionsInfiniteTable, { withRef: true }));
